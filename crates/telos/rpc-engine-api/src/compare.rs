...
use revm::db::AccountStatus as DBAccountStatus;
use revm::{Database, DatabaseCommit, Evm, State, TransitionAccount};
    ...
    for row in &statediffs_account {
        ...
        if let Ok(revm_row) = revm_db.basic(row.address) {
            if let Some(unwrapped_revm_row) = revm_row {
                let mut mod_acc = Account {
                    info: unwrapped_revm_row.clone(),
                    storage: HashMap::new(),
                ...
                if unwrapped_revm_row.balance != row.balance {
                    ...
                    mod_acc.info.balance = row.balance;
                    modded = true;
                }
                    ...
                    match revm_db.code_by_hash(unwrapped_revm_row.code_hash) {
                        Ok(code_by_hash) =>
                            if (code_by_hash.is_empty() && row.code.len() != 0) || (!code_by_hash.is_empty() && row.code.len() == 0) {
                                ...
                                if row.code.len() == 0 {
                                    mod_acc.info.code = None;
                                } else {
                                    mod_acc.info.code = Some(Bytecode::LegacyRaw(row.code.clone()));
                                }
                                modded = true;
                            },
                        Err(_) => {
                            if panic_mode {
                                panic!("Difference in code existence, address: {:?} - revm: {:?} - tevm: {:?}", row.address, unwrapped_revm_row.code, row.code)
                            }
                            if row.code.len() == 0 {
                                mod_acc.info.code = None;
                            } else {
                ...
                if modded {
                    ...
                    mod_state.insert(row.address, mod_acc);
